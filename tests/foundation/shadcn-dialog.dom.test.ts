...
import DialogFixture from "@/components/ui/__test_helper__/DialogFixture.vue";

describe("Phase 0 foundation — shadcn-vue dialog 底座", () => {
  it("DialogPortal 被 stub，content 留在 wrapper 内且 data-state=open", () => {
    const wrapper = mountWithProviders(DialogFixture, { attachTo: document.body });

    expect(wrapper.find("[data-portal-stub]").exists()).toBe(true);
  ...
  it("Button 把 $attrs（data-fn / aria-label）转发到内层 <button>，调用方 class 胜出", () => {
    ...
    const cancel = wrapper.find('button[data-fn="dialog-cancel"]');
  ...
  it("点 cancel 关掉 dialog", async () => {
    ...
    await wrapper.find('button[data-fn="dialog-cancel"]').trigger("click");
    await nextTick();
  ...
  it("Input 无条件绑定 disabled 且转发 $attrs；Label 转发 $attrs 并保留 for", () => {
    ...
    const input = wrapper.find('input[data-fn="dialog-input"]');
    ...
    const label = wrapper.find('label[data-fn="dialog-label"]');
